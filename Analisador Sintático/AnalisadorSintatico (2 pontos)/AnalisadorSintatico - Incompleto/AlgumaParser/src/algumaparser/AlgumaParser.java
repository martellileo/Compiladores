...
public class AlgumaParser {
    ...
    void fatorAritmetico() {
        if(lookahead(1).nome == TipoToken.NumInt)
            ...
            erroSintatico(
                    TipoToken.NumInt.toString(), 
                    TipoToken.NumReal.toString(), 
    ...
    void expressaoRelacional2() {
        // teste e verifique qual o tamanho do lookahead. Verifique os operadores booleanos no TipoToken
        if(lookahead(1).nome == TipoToken.OpBoolE 
                || lookahead(1).nome == TipoToken.OpBoolOu){
            operadorBooleano();
            termoRelacional();
            expressaoRelacional2();
        } else{
    ...
    void comando() {
        if(lookahead(1).nome == TipoToken.PCAtribuir)
            ...
            erroSintatico("comandoAtribuicao",
                    "comandoEntrada",
                    "comandoSaida", 
                    "ComandoCondicao", 
                    "comandoRepeticao", 
